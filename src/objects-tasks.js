...
function compareObjects(obj1, obj2) {
  const str1 = Object.entries(obj1).flat().join('');
  const str2 = Object.entries(obj2).flat().join('');
  return str1 === str2;
}
...
function sellTickets(queue) {
  ...
  const checkChange = !queue.some((el) => {
    if (el === 25) {
      cost25 += 1;
    } else if (el === 50) {
      if (cost25 > 0) {
...
function sortCitiesArray(arr) {
  const arrSort = arr.sort(
    (a, b) =>
      a.country.localeCompare(b.country, 'us') ||
      a.city.localeCompare(b.city, 'us')
  );
  return arrSort;
}
